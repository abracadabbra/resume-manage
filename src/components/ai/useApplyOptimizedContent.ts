...
import { getModuleLabel } from '@/services/aiService'

interface UseApplyOptimizedContentOptions {
  selectedModule: Ref<string>
  resolvedOptimizedContent: Ref<string>
  markdown: MarkdownIt
...
export function useApplyOptimizedContent(options: UseApplyOptimizedContentOptions) {
  const resumeStore = useResumeStore()
  const { selectedModule, resolvedOptimizedContent, markdown, setErrorMessage } = options
  ...
  const isSelectedModuleApplied = computed(() => appliedModules.value.has(selectedModule.value))
  ...
  const applySupportedModules = new Set([
    'skills',
    'selfIntro',
    'workExperience',
    'projectExperience',
  ...
  const PROJECT_RESULT_PHRASE_REGEX =
    /从\s*\d+(?:\.\d+)?\s*(?:ms|s|秒|分钟|小时|天|周|月|年|QPS|TPS|%|倍|个|次|项|万|亿)?\s*(?:左右)?\s*(?:降至|降低到|下降到|减少到|优化到|提升至|提升到|提高到)\s*\d+(?:\.\d+)?\s*(?:ms|s|秒|分钟|小时|天|周|月|年|QPS|TPS|%|倍|个|次|项|万|亿)?/gi
  const PROJECT_METRIC_REGEX =
    /\d+(?:\.\d+)?\s*(?:ms|s|秒|分钟|小时|天|周|月|年|QPS|TPS|%|倍|个|次|项|w|W|万|亿|封\/(?:分钟|小时|天)|条\/s|次\/s)/g
  ...
  function mergeShortSkillsListItems(markdownText: string): string {
    ...
    const getItemInfo = (
      line: string,
    ): { marker: string; body: string; level: string; shortScore: number } | null => {
      ...
      const shortScore = stripMarkdownDecorators(bodyWithoutLevel).replace(
        /[，。；、,.;:\s]/g,
        '',
      ).length
    ...
    for (const rawLine of lines) {
      ...
      const sameListType =
        /^(\s*[-*+]\s+)/.test(prevInfo.marker) === /^(\s*[-*+]\s+)/.test(currentInfo.marker)
      ...
      const mergedBodyLength = stripMarkdownDecorators(
        `${prevInfo.body}${currentInfo.body}`,
      ).replace(/\s+/g, '').length
  ...
  function wrapRegexMatchesAsBoldToken(
    text: string,
    regex: RegExp,
  ...
  function wrapKeywordsAsBoldToken(
    text: string,
  ...
  function removeMarkdownStrongMarkers(markdownText: string): string {
    return markdownText.replace(/\*\*([^*]+)\*\*/g, '$1').replace(/__([^_]+)__/g, '$1')
  }
  ...
  function parseProjectOptimizedContent(markdownText: string): {
    introduction: string
    mainWork: string
  } {
    ...
    const isWorkHeading = (line: string): boolean => {
      const plain = stripMarkdownDecorators(line).replace(/[：:]$/, '')
      return (
        plain === '主要工作' || plain === '核心职责' || plain === '工作内容' || plain === '职责'
      )
    }
  ...
  function isProjectMetadataLine(line: string, projectList: ProjectEntry[]): boolean {
    ...
    const hasDateRange =
      /(?:19|20)\d{2}[./-]\d{1,2}\s*(?:[~～\-—–至到]+\s*(?:(?:19|20)\d{2}[./-]\d{1,2}|至今|现在))/.test(
        plain,
      )
  ...
  function parseProjectOptimizedSections(
    markdownText: string,
    projectList: ProjectEntry[],
  ): Array<{ introduction: string; mainWork: string }> {
    const lines = markdownText.replace(/\r\n/g, '\n').split('\n')
    const projectNames = projectList.map((p) => p.name.trim()).filter(Boolean)
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const hasMetaSeparator = /[|｜•·]/.test(plain)
      const isSectionHeading =
        /^#{1,6}\s*项目经历\s*\d+/i.test(raw) || /^项目经历\s*\d+[：:]?$/i.test(plain)

      if (
        isSectionHeading ||
        (hasProjectName &&
          (isProjectNameOnly || hasDateRange || hasMetaSeparator || /^#{1,6}\s+/.test(raw)))
      ) {
  ...
  function parseWorkOptimizedContent(markdownText: string, workList: WorkEntry[]): string[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const hasMetaSeparator = /[|｜•·]/.test(plain)
      const isSectionHeading =
        /^#{1,6}\s*工作经历\s*\d+/i.test(raw) || /^工作经历\s*\d+[：:]?$/i.test(plain)

      if (
        isSectionHeading ||
        (hasCompany &&
          (isCompanyOnly || hasDateRange || hasMetaSeparator || /^#{1,6}\s+/.test(raw)))
      ) {
        sectionStartIndexes.push(i)
      }
    }
  ...
  function parseAwardFieldLine(
    line: string,
  ): { field: 'name' | 'date' | 'description'; value: string } | null {
  ...
  function normalizeMarkdownListType(
    markdownText: string,
    preferredListType: PreferredListType | null,
  ): string {
  ...
  function renderWorkSectionWithOriginalStyle(
  ...
  function renderProjectMainWorkWithOriginalStyle(
    sectionMarkdown: string,
    originalHtml: string,
  ): string {
  ...
  function renderAwardDescriptionWithOriginalStyle(
    sectionMarkdown: string,
    originalHtml: string,
  ): string {
  ...
  function handleApply() {
      ...
      case 'workExperience':
        if (resumeStore.workList.length > 0 && resumeStore.workList[0]) {
          ...
          if (!applied && content.trim()) {
            ...
            if (firstWork) {
              firstWork.description = renderWorkSectionWithOriginalStyle(
                sanitized,
        ...
        if (resumeStore.projectList.length > 0 && resumeStore.projectList[0]) {
          ...
          for (let i = 0; i < maxApplyCount; i++) {
            ...
            if (section.mainWork) {
              ...
              project.mainWork = renderProjectMainWorkWithOriginalStyle(
                enhancedMainWork,
                project.mainWork,
              )
              applied = true
          ...
          if (!applied && content.trim()) {
            ...
            target.mainWork = renderProjectMainWorkWithOriginalStyle(
              enhancedMainWork,
              target.mainWork,
            )
            applied = true
    ...
    if (!applied) {
      setErrorMessage(`「${getModuleLabel(key)}」暂不支持一键应用，请先复制后手动调整。`)
      return
    }
  ...
  function handleUndoApply() {
    ...
    switch (key) {
      ...
      case 'workExperience':
        if (snapshot.previousWorkDescriptions?.length) {
          const restoreCount = Math.min(
            snapshot.previousWorkDescriptions.length,
            resumeStore.workList.length,
          )
      ...
      case 'projectExperience':
        if (
          snapshot.previousProjectMainWorks?.length ||
          snapshot.previousProjectIntroductions?.length
        ) {
          const restoreCount = Math.min(
  ...
  function resetAppliedModule(moduleKey: string) {
